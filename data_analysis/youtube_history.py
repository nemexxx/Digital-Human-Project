...
def run_youtube_search_analysis():
    ...
    with open(data_path("YouTube", "Verlauf", "Suchverlauf.json"), "r") as file:
...
def run_youtube_watch_analysis():
    ...
    with open(data_path("YouTube", "Verlauf", "Wiedergabeverlauf.json"), "r") as file:
...
if __name__ == "__main__":
    search_result = run_youtube_search_analysis()
    watch_result = run_youtube_watch_analysis()

    print("Most Searched Keywords:")
    for keyword, count in search_result["most_searched_keywords"]:
        print(f"{keyword}: {count} searches")

    print(f"\nPeak YouTube Search Hour: {search_result['peak_search_hour']}")

    print("\nTop 5 Most Watched Channels:")
    for channel, count in watch_result["top_5_watched_channels"]:
        print(f"{channel}: {count} views")

    print(f"\nPeak YouTube Watch Hour: {watch_result['peak_watch_hour']}")