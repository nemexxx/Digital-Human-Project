...
def run_chrome_history_analysis():
    ...
    with open(data_path("Chrome", "verlauf.json"), "r") as file:
...
if __name__ == "__main__":
    result = run_chrome_history_analysis()

    print("Top 5 Most Visited Domains:")
    for domain, count in result["top_5_visited_domains"]:
        print(f"{domain}: {count} visits")

    print("\nTop 5 Most Frequent Titles:")
    for title, count in result["top_5_frequent_titles"]:
        print(f"{title}: {count} visits")

    print(f"\nOverall Peak Browsing Hour: {result['peak_browsing_hour_overall']}")